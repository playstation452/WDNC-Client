            ...
            if (e.getPacket() instanceof SPacketBlockBreakAnim) {
                ...
                if ((mode.getMode("Self").isToggled()) && isPlayersHoleBlock(mc.player, sPacketBlockBreakAnim.getPosition())) {
                    ...
                    if (notifMode.getMode("Notification").isToggled()) {
                        new Notification("HoleBreakNotifier", Objects.requireNonNull(mc.world.getEntityByID(sPacketBlockBreakAnim.getBreakerId())).getName() + " this fag is breaking your hole!", 5000, Color.BLACK);
                    } else if (notifMode.getMode("Chat").isToggled()) {
                        mc.player.sendChatMessage(Objects.requireNonNull(mc.world.getEntityByID(sPacketBlockBreakAnim.getBreakerId())).getName() + " this fag is trying to break my hole");
                    } else if (notifMode.getMode("ClientSideChat").isToggled()) {
                        ChatUtils.normalMessage(Objects.requireNonNull(mc.world.getEntityByID(sPacketBlockBreakAnim.getBreakerId())).getName() + " WAKE UP, THIS FAG IS BREAKING YOUR HOLE!");
                    }
                }
